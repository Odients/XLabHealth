...
#### 2.1.2.1. Логика определения общего статуса системы (с учётом критичности)
Каждый сервис имеет флаг **IsCritical** (критичность). Общий статус системы определяется по следующим правилам:
- **Unhealthy** (вся система не работает): если хотя бы один критический сервис не работает (Unhealthy или Unknown)
- **Degraded** (система ограниченно функционирует): если критический сервис деградирован, либо не работают только некритичные сервисы
- **Healthy** (система работает): все сервисы в состоянии Healthy
