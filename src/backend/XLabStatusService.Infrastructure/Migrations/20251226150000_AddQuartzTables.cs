...
namespace XLabStatusService.Infrastructure.Migrations
{
    ...
    public partial class AddQuartzTables : Migration
    {
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            // Удаляем индексы
            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_FT_TG' AND object_id = OBJECT_ID('QRTZ_FIRED_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_FT_TG] ON [dbo].[QRTZ_FIRED_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_FT_T_G' AND object_id = OBJECT_ID('QRTZ_FIRED_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_FT_T_G] ON [dbo].[QRTZ_FIRED_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_FT_JG' AND object_id = OBJECT_ID('QRTZ_FIRED_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_FT_JG] ON [dbo].[QRTZ_FIRED_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_FT_J_G' AND object_id = OBJECT_ID('QRTZ_FIRED_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_FT_J_G] ON [dbo].[QRTZ_FIRED_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_FT_INST_JOB_REQ_RCVRY' AND object_id = OBJECT_ID('QRTZ_FIRED_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_FT_INST_JOB_REQ_RCVRY] ON [dbo].[QRTZ_FIRED_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_FT_TRIG_INST_NAME' AND object_id = OBJECT_ID('QRTZ_FIRED_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_FT_TRIG_INST_NAME] ON [dbo].[QRTZ_FIRED_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_NFT_ST_MISFIRE' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_NFT_ST_MISFIRE] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_NFT_MISFIRE' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_NFT_MISFIRE] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_NFT_ST' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_NFT_ST] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_NEXT_FIRE_TIME' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_NEXT_FIRE_TIME] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_N_G_STATE' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_N_G_STATE] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_N_STATE' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_N_STATE] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_STATE' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_STATE] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_G' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_G] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_C' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_C] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_JG' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_JG] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_T_J' AND object_id = OBJECT_ID('QRTZ_TRIGGERS'))
                    DROP INDEX [IDX_QRTZ_T_J] ON [dbo].[QRTZ_TRIGGERS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_J_GRP' AND object_id = OBJECT_ID('QRTZ_JOB_DETAILS'))
                    DROP INDEX [IDX_QRTZ_J_GRP] ON [dbo].[QRTZ_JOB_DETAILS]
            ");

            migrationBuilder.Sql(@"
                IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_QRTZ_J_REQ_RECOVERY' AND object_id = OBJECT_ID('QRTZ_JOB_DETAILS'))
                    DROP INDEX [IDX_QRTZ_J_REQ_RECOVERY] ON [dbo].[QRTZ_JOB_DETAILS]
            ");

            // Удаляем таблицы в обратном порядке (сначала зависимые)
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_LOCKS]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_SCHEDULER_STATE]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_FIRED_TRIGGERS]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_PAUSED_TRIGGER_GRPS]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_CALENDARS]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_BLOB_TRIGGERS]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_SIMPROP_TRIGGERS]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_CRON_TRIGGERS]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_SIMPLE_TRIGGERS]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_TRIGGERS]");
            migrationBuilder.Sql("DROP TABLE IF EXISTS [dbo].[QRTZ_JOB_DETAILS]");
        }
    }
}
